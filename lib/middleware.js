...
const createInMemoryAdapter = require('./inMemoryAdapter').create
...
function createStateMiddleware(
  {
    logger = basicLogger, 
    dataAdapter = createInMemoryAdapter()
  } = {}
) {
  return function middleware(socket, next) { 
    socket.on(EVENTS.JOIN_TO_GROUP, ({groupId}, ack) => {
      ...
      socket.join(groupId, (error) => {
        ...
        const currentState = dataAdapter.get(groupId)
    ...
    socket.on(EVENTS.UPDATE_STATE, (params, ack) => {
      ...
      try {
        const stateToSend = dataAdapter.update(groupId, updateId, newState)
    ...
    socket.on(EVENTS.GET_STATE, ({ groupId }, ack) => {
      ...
      const stateToSend = dataAdapter.get(groupId)
      sendAck(stateToSend, ack)
    })
    next()
  }
...
module.exports = { createStateMiddleware, EVENTS }
